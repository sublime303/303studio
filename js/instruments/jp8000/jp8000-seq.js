(function(global) {
  ...
  class JP8000Seq {
    ...
    _blank() {
      return Array.from({length:16}, () => ({ root:'C', octave:0, rest:true }));
    }
    play(ctx, dest) {
    ...
    _onTick(step, time) {
      if (step === -1) { this.eng.allNotesOff(global.Bus.now); if (this.onStep) this.onStep(-1); return; }
    ...
    _fire(idx, time) {
      ...
      this.eng.chordOn(rootMidi, time);
      if (next.rest) this.eng.allNotesOff(time + dur * 0.92);
    }
  }

  global.JP8000Seq = JP8000Seq;
